                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {prop.availabilityStatus === 'rented' ? (
                            <span className="text-[10px] font-black px-2 py-0.5 rounded-md bg-rose-600 text-white flex items-center gap-1">
                              <KeyRound className="w-3 h-3" />
                              <span>{language === 'am' ? 'ተከራይቷል (Rented)' : 'Rented'}</span>
                            </span>
                          ) : prop.availabilityStatus === 'sold' ? (
                            <span className="text-[10px] font-black px-2 py-0.5 rounded-md bg-purple-700 text-white flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3" />
                              <span>{language === 'am' ? 'ተሸጧል (Sold)' : 'Sold'}</span>
                            </span>
                          ) : (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 flex items-center gap-1 border border-emerald-300">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-600 animate-pulse" />
                              <span>{language === 'am' ? 'ክፍት ነው' : 'Available'}</span>
                            </span>
                          )}
                        </div>
                        ...
                        {/* Quick Status Toggle for Landlord */}
                        <div className="mt-2 flex items-center gap-1 text-[11px]">
                          <span className="text-slate-400 font-medium mr-1">{language === 'am' ? 'ሁኔታ ቀይር:' : 'Set Status:'}</span>
                          <button
                            type="button"
                            onClick={() => updateProperty(prop.id, { availabilityStatus: 'available' })}
                            className={`px-2 py-0.5 rounded text-[10px] font-bold cursor-pointer transition-colors ${
                              (!prop.availabilityStatus || prop.availabilityStatus === 'available')
                          ...
                          <button
                            type="button"
                            onClick={() => updateProperty(prop.id, { availabilityStatus: 'rented' })}
                            className={`px-2 py-0.5 rounded text-[10px] font-bold cursor-pointer transition-colors ${
                              prop.availabilityStatus === 'rented'
                          ...
                          >
                            🔑 {language === 'am' ? 'ተከራይቷል' : 'Rented'}
                          </button>
                          <button
                            type="button"
                            onClick={() => updateProperty(prop.id, { availabilityStatus: 'sold' })}
                            className={`px-2 py-0.5 rounded text-[10px] font-bold cursor-pointer transition-colors ${
                              prop.availabilityStatus === 'sold'
